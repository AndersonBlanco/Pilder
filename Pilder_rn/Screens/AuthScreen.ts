import { View, Text, Image, SafeAreaView, StyleSheet, ImageBackground, TextInput, TouchableOpacity, Dimensions } from "react-native";
...
interface Styles {
  ...
  textInput: TextStyle;
  authButtons: ViewStyle;
  authButtons_text: TextStyle;
  hotFix1: ViewStyle;
  row: ViewStyle;
}
...
export default function AuthScreen(): JSX.Element {
    ...
    const LogoImg: JSX.Element = (
        <View style={styles.logoContainer}>
            <Image source={Logo} style={styles.logo} />
        </View>
    );

    const Interactables: JSX.Element = (
        <View style={{ top: -37, rowGap: 50 }}>
            <Text style={{ fontSize: 14, fontWeight: "300" }}>
                Forgot username or password? 
                <TouchableOpacity>
                    <Text style={{ bottom: -5, color: "rgba(250, 84,84,1)", textDecorationLine: "underline" }}>
                        Reset
                    </Text>
                </TouchableOpacity>
            </Text>
    ...
    const LogIn: JSX.Element = (
        <View style={[styles.column, { backgroundColor: "transparent", padding: 50, alignItems: "center", rowGap: 40, bottom: -70 }]}>
            ...
            <Text style={{ fontSize: 14, fontWeight: "300" }}>
                ...
                <TouchableOpacity onPress={() => setAuthType(false)}>
                    <Text style={{ bottom: -5, color: "rgba(250, 84,84,1)", textDecorationLine: "underline" }}>
                        SignUp
                    </Text>
                </TouchableOpacity>
    ...
    const SignUp: JSX.Element = (
        <View style={[styles.column, { backgroundColor: "transparent", padding: 50, alignItems: "center", rowGap: 50, bottom: -20 }]}>
            ...
            <TouchableOpacity style={styles.authButtons} onPress={() => {/* nav */}}>
                <Text style={styles.authButtons_text}>SignUp</Text>
            </TouchableOpacity>

            <Text style={{ fontSize: 14, fontWeight: "300" }}>
                Already have an account? 
                <TouchableOpacity onPress={() => setAuthType(true)}>
                    <Text style={{ bottom: -5, color: "rgba(250, 84,84,1)", textDecorationLine: "underline" }}>
                        SignIn
                    </Text>
                </TouchableOpacity>
